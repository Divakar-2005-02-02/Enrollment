...
const AssignSubjectStaff = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const subjectResponse = await axios.get('http://51.20.18.18:8080/admin/sub'); // URL to fetch subjects
        setSubjects(subjectResponse.data);

        const staffResponse = await axios.get('http://51.20.18.18:8080/staff/disp-staff'); // URL to fetch staff
  ...
  const handleAssign = async () => {
    ...
    try {
      await axios.post('http://51.20.18.18:8080/admin/assign-subject', assignmentData);
