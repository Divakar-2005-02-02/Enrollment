...
const StudentsManagement = () => {
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get(
        "http://51.20.18.18:8080/admin/display-students"
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://51.20.18.18:8080/admin/add-student", formData);
  ...
  const handleDelete = async (rollNumber) => {
    ...
    if (confirmed) {
      try {
        await axios.delete(
          `http://51.20.18.18:8080/admin/delete-student/${rollNumber}`
        );
  ...
  const handleFileUpload = async () => {
    if (file) {
      ...
      try {
        await axios.post("http://51.20.18.18:8080/admin/upload-excel", formData);
        fetchStudents();
