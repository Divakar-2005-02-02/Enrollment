...
const StaffManagement = () => {
  ...
  const fetchStaff = async () => {
    try {
      const response = await axios.get('http://51.20.18.18:8080/staff/disp-staff');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://51.20.18.18:8080/admin/add-staff', formData);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        await axios.delete(`http://51.20.18.18:8080/admin/remove-staff/${id}`);
