...
const StaffLogin = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const loginResponse = await axios.post('http://51.20.18.18:8080/admin/staff/login', { email, password });

      if (loginResponse.status === 200) {
        const response = await axios.get(`http://51.20.18.18:8080/staff/get-id/${email}`);
