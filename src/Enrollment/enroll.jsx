...
const EnrollmentPage = () => {
    ...
    const fetchStudentDetails = async () => {
      try {
        const response = await axios.get(
          `http://51.20.18.18:8080/students/get-student/${rollNumber}`
        );
        setStudentDetails(response.data);
        setStudentId(response.data.id);
        const subjectsResponse = await axios.get(
          `http://51.20.18.18:8080/admin/get-sub?year=${response.data.yearOfStudy}&semester=${response.data.semester}`
        );
        setSubjectStaffDetails(subjectsResponse.data);

        const enrolledResponse = await axios.get(
          `http://51.20.18.18:8080/enroll/get-enrolled-subjects1/${response.data.id}`
        );
  ...
  const handleEnroll = async (subjectId, staffId) => {
    ...
    try {
      ...
      const response = await axios.post(`http://51.20.18.18:8080/enroll/subjects`, enrollmentData);
  ...
  const handleUnenroll = async (subjectId) => {
    ...
    try {
      const response = await axios.delete(
        `http://51.20.18.18:8080/enroll/remove-subject/${studentId}/${subjectId}`
      );
